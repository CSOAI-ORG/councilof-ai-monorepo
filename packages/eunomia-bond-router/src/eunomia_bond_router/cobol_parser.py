...
def _parse_pic(pic):
    ...
    if m:
        # PIC 9(12)V99 = 12 integer digits + 2 implied decimal places. scale =
        # COUNT of decimal digit positions (len of "99"), NOT its numeric value.
        return "decimal", int(m.group(1)) + len(m.group(2)), len(m.group(2))
    m = re.search(r'(9+)\s*V\s*(\d+)', pic.upper())
    if m:
        return "decimal", len(m.group(1)), len(m.group(2))
